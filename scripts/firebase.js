...
function realtimeData() {
  ...
  onValue(ref(db, "realtimeData/"), (snapshot) => {
    ...
    document.getElementById("diameterData").innerHTML =
      snapshot.val().diameter.toFixed(2) + " cm";
...
function elList() {
  historyList.innerHTML = `
      ${child_data.map((cyclingData) => {
        return `
            <div class="history-content-title flex">
              <div class="history-content-date">
                ${cyclingData.date} ${cyclingData.startTime}
              </div>
              <div class="grid-history grid-col-2-history">
                <div  class="history-content">Distance: ${cyclingData.distance}</div>
                <div  class="history-content">Max Speed: ${cyclingData.maxSpeed}</div>
                <div  class="history-content">Average Speed: ${cyclingData.averageSpeed}</div>
                <div  class="history-content">Elapsed Time: ${cyclingData.elapsedTime}</div>
              </div>
            </div>
          `;
      })}
...
setDiameterBtn.addEventListener("click", function () {
  ...
  document.getElementById("inputDiameter").value = 0;
});
